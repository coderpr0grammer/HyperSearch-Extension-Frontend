...
const HamburgerMenu = () => {
  ...
  const { user, subscribedToPro, userData, isAdmin } = useContext(AuthenticationContext);
  ...
  const openSubPortal = async () => {
    ...
    const { data } = await functionRef({
      returnUrl: vid ? `https://youtube.com/watch?v=${vid}` : "https://youtube.com",
    });
  ...
  const openSubCheckout = async () => {
    ...
    const docRef = await addDoc(checkout_sessions_ref, {
      price: "price_1N9SoGGYsk0yMBHYmrjNWpIX",
      success_url: vid ? `https://youtube.com/watch?v=${vid}` : "https://youtube.com",
    });
      ...
      <ul className={`menu ${isOpen && "show"}`} ref={menuRef}>
        ...
        {user && (
          <li className={isOpen ? "show" : ""}>
            <button className="userProfile"><span style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', fontFamily: '90%' }}>{user.photoURL ? <img src={user.photoURL} width="24" height="24" style={{ borderRadius: 5, marginRight: 5 }} /> : <FontAwesomeIcon icon={faUser} />} {user.displayName}</span><span style={{ fontSize: '80%' }}>{user.email}</span></button>
          </li>
        )}

        {subscribedToPro && (
          <li className={isOpen ? "show" : ""}>
            <FontAwesomeIcon icon={faCrown} color="#688EF9" />
          </li>
        )}

        {!subscribedToPro && !isAdmin && (
          <li className={isOpen ? "show" : ""}>
            <button onClick={openSubCheckout}
              style={{
                background:
            ...
            >
              <FontAwesomeIcon icon={faCrown} color="#158CE6" style={{background:
                ...
                WebkitBackgroundClip: 'text',
                WebkitTextFillColor: 'transparent'}}/> Upgrade to Pro
            </button>
          </li>
        )}
        <li className={isOpen ? "show" : ""}>
